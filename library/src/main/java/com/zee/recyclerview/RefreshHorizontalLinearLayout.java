 ...
 */
class RefreshHorizontalLinearLayout extends LinearLayout implements IRefreshLinearLayout {
     ...
     * @param attrs
     */
    public RefreshHorizontalLinearLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public boolean isWaitRefresh() {
        return mState < RefreshHorizontalLinearLayout.STATE_REFRESH_START;
    }
        ...
        if (isRefreshEnabled()) {
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    final float deltaY = ev.getRawX() - mLastX;
                    mLastX = ev.getRawX();
                    if (isOnTop() && isRefreshEnabled() && appbarState == AppBarStateChangeListener.State.EXPANDED) {
                        ...
                        if (getVisibleHeight() > 0 && getState() < RefreshHorizontalLinearLayout.STATE_REFRESH_START) {
                            return false;
                        }
                    }
                    break;
...
//            }, 200);
    ...
    public void setVisibleHeight(int height) {
        ...
        if (height < customHeight) {
            mContainer.setTranslationX(-(customHeight - height) / 2);
        } else {
