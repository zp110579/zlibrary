...
public class ZxSolidTabLayout extends LinearLayout implements ValueAnimator.AnimatorUpdateListener, ViewPager.OnPageChangeListener {
    ...
    public ZxSolidTabLayout(Context context) {
        this(context, null, 0);
    }

    public ZxSolidTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ZxSolidTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //create ViewPager
        if (height == null) {
            throw new ZException("ZxSolidTabLayout 没有设置layout_height");
        } else if (height.equals(ViewGroup.LayoutParams.MATCH_PARENT + "")) {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ZxSolidTabLayout);

        mIndicatorColor = ta.getColor(R.styleable.ZxSolidTabLayout_zv_indicator_color, Color.parseColor("#222831"));
        mIndicatorHeight = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_indicator_height, -1);
        mIndicatorCornerRadius = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_radius, -1);
        mIndicatorMargin = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_indicator_margin, dp2px(0));
        mIndicatorMarginLeft = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_indicator_margin_left, dp2px(0) + mIndicatorMargin);
        mIndicatorMarginTop = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_indicator_margin_top, mIndicatorMargin);
        mIndicatorMarginRight = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_indicator_margin_right, dp2px(0) + mIndicatorMargin);
        mIndicatorMarginBottom = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_indicator_margin_bottom, mIndicatorMargin);
        mIndicatorAnimEnable = ta.getBoolean(R.styleable.ZxSolidTabLayout_zv_indicator_animal, false);
        mIndicatorBounceEnable = ta.getBoolean(R.styleable.ZxSolidTabLayout_zv_indicator_bounce_enable, true);
        mIndicatorAnimDuration = ta.getInt(R.styleable.ZxSolidTabLayout_zv_indicator_anim_duration, -1);

        mDividerColor = ta.getColor(R.styleable.ZxSolidTabLayout_zv_divider_color, mIndicatorColor);
        mDividerWidth = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_divider_width, dp2px(1));
        mDividerPadding = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_divider_padding, 0);

        mTextSize = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_textSize, sp2px(13f));
        mTextSelectColor = ta.getColor(R.styleable.ZxSolidTabLayout_zv_textSelectColor, Color.parseColor("#ffffff"));
        mTextUnSelectColor = ta.getColor(R.styleable.ZxSolidTabLayout_zv_textColor, mIndicatorColor);
        mTextBold = ta.getInt(R.styleable.ZxSolidTabLayout_zv_textBold, TEXT_BOLD_NONE);
        mTextAllCaps = ta.getBoolean(R.styleable.ZxSolidTabLayout_zv_textAllCaps, false);

        mTabSpaceEqual = ta.getBoolean(R.styleable.ZxSolidTabLayout_zv_tab_space_equal, false);
        mTabWidth = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_tab_width, dp2px(-1));
        mTabPadding = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_tab_padding, mTabSpaceEqual || mTabWidth > 0 ? dp2px(0) : dp2px(10));

        mBarColor = ta.getColor(R.styleable.ZxSolidTabLayout_zv_bar_color, Color.TRANSPARENT);
        mBorderColor = ta.getColor(R.styleable.ZxSolidTabLayout_zv_borderColor, mIndicatorColor);
        mBorderWidth = ta.getDimension(R.styleable.ZxSolidTabLayout_zv_borderWidth, dp2px(1));

        //以“|”分割
        String title = ta.getString(R.styleable.ZxSolidTabLayout_zv_title_text);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public void setViewPager(final ViewPager viewPage, String[] titles) {
        if (titles == null) {
            ...
            if (count > 0 && !TextUtils.isEmpty(adapter.getPageTitle(0))) {
