...
class PassWordView extends View {
    ...
    public void setBox_radius(float box_radius) {
    ...
    public void setPassText(String mPassText) {
    ...
    public void setBoxDrawType(int mBoxDrawType) {
        this.mBoxDrawType = mBoxDrawType;
    }


    public void setShowPassType(int mShowPassType) {
        this.mShowPassType = mShowPassType;
    }

    public void setDrawTxtSize(int mDrawTxtSize) {
        this.mDrawTxtSize = mDrawTxtSize;
    }


    public void setIsShowRemindLine(boolean mIsShowShan) {
        this.isShowRemindLine = mIsShowShan;
    }

    public void setDrawBoxLineSize(int mDrawBoxLineSize) {
    ...
    private void drawInputTextOrPicture(Canvas canvas) {
        if (isDrawText) {            //是否需要进行绘制
            ...
            mPaint.setColor(mInputStateTextColor);
    ...
    private void drawRemindLine(Canvas canvas) {
        if (mDrawRemindLineState && isShowRemindLine) {  // mDrawRemindLineState 控制闪烁情况  //isShowRemindLine 是否绘制提示线
            int line_height = getMeasuredWidth() / 2 - 2;

            line_height = line_height < 0 ? getMeasuredWidth() / 2 : line_height;

            mPaint.setStyle(Paint.Style.FILL);//实心
            mPaint.setColor(mRemindLineColor);
    ...
    private void drawInputBox(Canvas canvas) {
        if (isInputState) { //是否是输入状态  输入状态和未输入状态颜色区分
            mPaint.setColor(mInputStateBoxColor);
        } else {
            mPaint.setColor( mNoInputStateBoxColor);
        }
        ...
        switch (mBoxDrawType) {
            ...
            case 2:        //绘制横线
                ...
                break;
            default:// 绘制矩形
                RectF rect = new RectF(box_radius / 2, box_radius / 2, getMeasuredWidth() - box_radius / 2, getMeasuredHeight() - box_radius / 2);
                canvas.drawRoundRect(rect, box_radius / 2, box_radius / 2, mPaint);
        }
    }
